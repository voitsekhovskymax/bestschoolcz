                ...
                <Card>
                    <Form :label-width="100">
                        <FormItem label="Название курса">
                            <Input v-model="course.title" @on-blur="titleBlur" icon="android-list"
                                   maxlength=256/></Input>
                        </FormItem>
                        ...
                        <FormItem label="Описание курса">
                            <Input v-model="course.description" type="textarea" :rows="4" placeholder="Описание курса"
                                   maxlength=256
                                   @on-blur="titleDescription"></Input>
                        </FormItem>
                    </Form>
                    <div class="margin-top-20">
                        <textarea id="articleCourseUpdate"></textarea>
                    </div>
                </Card>
            </div>
            </Col>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                course: {
                    ...
                    // тип курса, семестровый | годичный. Если выбран семестровый тип,  поле last_semester будет пустым.
                    //  [semester || year]
        ...
        destroyed() {
            tinymce.get('articleCourseUpdate').destroy();
        },
        methods: {
            addCategory() {
            ...
            getCourse() {
                this.axios.get('/api/courses/' + this.$route.params.id + '/edit').then((response) => {
                    ...
                    tinymce.init({
                        ...
                        setup: function (editor) {
                            editor.on('init', function (e) {
                                vm.spinShow = false;
                                tinymce.get('articleCourseUpdate').setContent(vm.course.content);
            ...
            titleBlur() {
                if (this.course.title.length !== 0) {
                    ...
                    this.$Message.error('Название курса не может быть пустым');
                }
            },
            titleDescription() {
            ...
            canPublish() {
                if (this.course.title.length === 0) {
                    this.$Message.error('Введите название курса');
                    return false;
                } else {
                    return true;
                }
            ...
            publish() {
                if (this.canPublish()) {
                    ...
                    this.course.content = tinymce.get('articleCourseUpdate').getContent();
                    this.publishLoading = true;
                    console.log(this.course);
                    this.axios.patch('/api/courses/' + this.$route.params.id, this.course).then((response) => {
                        console.log(response.data);
                        if (response.data == 0) {
                            this.publishLoading = false;
                            this.$Notice.warning({
