    ...
    <div class="post_list">
        <Row>
            ...
            <Card class="margin-top-10">
                <div class="photo_container" v-for="item in gallery.gallery_items">
                    <Row>
                        <Col span="12">
                        <div class="item_link">
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                gallery: {
                    ...
                    lang: 'ru',
                    url: '',
                    preview_img: '/assets/images/no-image.png',
                    type: 'photo',
                    categories: [],
                },
                fixedLink: '',
        ...
        methods: {
            deleteSlide(name) {
                for (var i = 0; i < this.gallery.gallery_items.length; i++) {
                    if (this.gallery.gallery_items[i]['local_name'] == name) {
                        this.gallery.gallery_items.splice(i, 1);
                    }
                }
            },
            addCategory() {
            ...
            addPhoto() {
                ...
                this.gallery.gallery_items.push(newPhoto);
                count_photos++;
            },
            titleBlur() {
            ...
            publish() {
                ...
                for (var i = 0; i < this.gallery.gallery_items.length; i++) {
                    let img_slide = this.gallery.gallery_items[i]['local_name'];
                    this.gallery.gallery_items[i].link = document.getElementById(img_slide).src;
                    if (this.gallery.gallery_items[i]['seo_title'] == "") {
                        this.gallery.gallery_items[i]['seo_title'] = this.gallery.gallery_items[i]['title']
                    }
                    if (this.gallery.gallery_items[i]['seo_alt'] == "") {
                        this.gallery.gallery_items[i]['seo_alt'] = this.gallery.gallery_items[i]['description']
                    }
                }

                console.log(this.gallery);
                this.axios.post('/api/galleries', this.gallery).then((response) => {
                    console.log(response.data);
                    if (response.data == 0) {
                        this.publishLoading = false;
                        this.$Notice.warning({
